...
2. [Деструктуризація.](#деструктуризація)
...
*Деструктуризація* (destructuring assignment) - це особливий синтаксис присвоєння, що дозволяє присвоїти масив чи об'єкт одраку декольком змінним, розбиваючи його на частини.

## Масив

Приклад деструктуризації масиву:

```js
'use strict';

let [firstName, lastName] = ["Іван", "Сірко"];

alert(firstName); // Іван
alert(lastName); // Сірко
```

При такому присвоєнні перше значення масиву піде в змінну `firstName`, друге - в `lastName`, а наступні (якщо є) - , будуть відкинуті.

Непотрібні, елементи масиву також можна відкинути, вказавши зайву кому:

```js
'use strict';

// перший та другий елементи масиву не потрібні
let [, , title] = "Сашко Положинський фронтмен гурту Тартак".split(" ");
```

В коді вище перший та другий елементи масиву нікуди не записалися, їх було відкинуто. Як, зрештою, і всі елементи що йдуть пясля третього.

## Оператор "spread"

У випадку, коли ми прагнемо отримати подальші значення масиву, але не впевнені у їхній кількості - можна додати ще один параметер, який отримає "все інше", за допомогою оператора `"..."` ("spread", трикрапки):

```js
'use strict';

let [firstName, lastName, ...rest] = "Сашко Положинський фронтмен гурту Тартак".split(" ");

alert(firstName); // Сашко
alert(lastName); // Положинський
alert(rest); // фронтмен гурту Тартак (масив з трьох елементів)
```

Значенням `rest` буде масив з елементів, що залишилися. Замість `rest` можна використовувати й інше імя змінної, оператор тут - трикрапки. Воно має стояти тільки останнім елеменом у списку зліва.

## Значення за замовчуванням

Якщо значень в масиві бульше, ніж змінних - помилки не трапиться, просто буде просвоєно `undefined`:
...
Втім, як правило, в таких випадках вказують значення за замовчуванням. Для цього необхідно після змінної використати символ `=` зі значенням, наприклад:

```js
let [firstName="Гість", lastName="Анонімний"] = [];

alert(firstName); // Гість
alert(lastName); // Анонімний
```

Як значення за замовчуванням можна використати не лише примітиви, але й вирази, ба навіть такі, що містять в собі виклики функцій:
...
// lastName отримає значення, що відповідає поточній даті:
let [firstName, lastName=defaultLastName()] = ["Іван"];

alert(firstName); // Іван
alert(lastName); // 1436...-visitor
```

Зауважимо, що виклик функції `defaultLastName` задля генерації значення за замовчуванням буде здійснено тільки за необхідності, тобто якщо значення немає в масиві.
